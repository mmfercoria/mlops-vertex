...
def choose_best_model(
    ...
    dt_pred = dt.predict(test_data.drop("target", axis=1))
    rf_pred = rf.predict(test_data.drop("target", axis=1))

    dt_accuracy = accuracy_score(test_data["target"], dt_pred)
    rf_accuracy = accuracy_score(test_data["target"], rf_pred)
